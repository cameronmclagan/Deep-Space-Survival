...
def leather_working_effect(rating, targets):
	plan_object, material_location = targets[:2]
	
	available_materials = material_location.recursive_content_list()
	used_materials = []
	
	for required_material_name in plan_object.required_materials:
		found_object = None
		for object in available_materials:
			if object.name == required_material_name:
				found_object = object
				break
		
		if found_object == None:
			terminal_game.add_line("Missing material: %s" % required_material_name)
		else:
			terminal_game.add_line("Found material: %s" % object.name)
			available_materials.remove(object)
			used_materials.append(object)
		
	for object in used_materials:
		object.move_to_container(None)
		
	global full_item_dict
	full_item_dict[plan_object.product].create_instance_at_location(material_location)
...
area_prototypes =	[
	["name",	"map_name",	"map_coords",	"area_id",	],
	["Cryo Recovery A",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_recovery_a"	],
	["Cryo Recovery B",	"citadel_cryo_wing",	(0.62,0.90),	"citadel_cryo_recovery_b"	],
	["Supply Room",	"citadel_cryo_wing",	(0.27,0.88),	"citadel_cryo_recovery_a_supply_room"	],
	["Supply Room",	"citadel_cryo_wing",	(0.73,0.88),	"citadel_cryo_recovery_b_supply_room"	],
	["Cryo Recovery Suite",	"citadel_cryo_wing",	(0.50,0.77),	"citadel_cryo_recovery_suite"	],
	["Supply Room",	"citadel_cryo_wing",	(0.50,0.90),	"citadel_cryo_recovery_suite_supply_room"	],
	["Cryonics General Care",	"citadel_cryo_wing",	(0.50,0.62),	"citadel_cryo_general_care"	],
	["Pre-Freeze Ward A",	"citadel_cryo_wing",	(0.27,0.62),	"citadel_cryo_pre_freeze_a"	],
	["Pre-Freeze Ward B",	"citadel_cryo_wing",	(0.73,0.62),	"citadel_cryo_pre_freeze_b"	],
	["Cryo Patient Prep A",	"citadel_cryo_wing",	(0.08,0.62),	"citadel_cryo_patient_prep_a"	],
	["Cryo Patient Prep B",	"citadel_cryo_wing",	(0.93,0.62),	"citadel_cryo_patient_prep_b"	],
	["Cryo Doctor Prep A",	"citadel_cryo_wing",	(0.27,0.77),	"citadel_cryo_doctor_prep_a"	],
	["Cryo Doctor Prep B",	"citadel_cryo_wing",	(0.73,0.77),	"citadel_cryo_doctor_prep_b"	],
	["Operating Room A",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_operating_room_a"	],
	["Operating Room B",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_operating_room_b"	],
	["Cryo Freezer A",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_freezer_a"	],
	["Cryo Freezer B",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_freezer_a"	],
	["Cryo Wing Lobby",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_lobby"	],
	["Physical Therapy Center",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_physical_therapy"	],
	["Post-Cryo Counseling Office",	"citadel_cryo_wing",	(0.38,0.90),	"citadel_cryo_counseling_office"	],
	]
...
area_descriptions =	{
	...
	"citadel_cryo_recovery_a_supply_room":	"This crowded supply room houses the many specialized supplies involved with reviving a frozen corpse. Due to their specialized nature they are essentially useless for any other application.",
	}
...
def fill_new_game_in_target_state(target_state):
	...
	for area_proto in area_prototypes[1:]:
		...
		if params["area_id"] in area_descriptions:
			area = mechanics.Area(name = name, description=area_descriptions[params["area_id"]])
		else:
			area = mechanics.Area(name = name)
	...
	for prop_def in prop_placement_table[1:]:
		...
		prop = mechanics.Scenery(location=target_state.area_dict[params["location"]],name=params["name"],description=params["description"])
		
		prop_marker = target_state.get_marker_for_identifier(params["prop_id"])
		prop_marker.focus_commands = mechanics.scenery_commands
	...
	for portal_proto in portal_prototypes[1:]:
		...
		portal = mechanics.Scenery(name=name, location=location, description=type_description)
		portal_marker = target_state.get_marker_for_identifier(portal.unique_name)
		portal_marker.visible_name = portal.formatted_name
		portal_marker.object = portal
		portal_marker.focus_commands = mechanics.scenery_commands
		portal_marker.move_to_marker(location_marker)
		
		move_target_marker = target_state.get_marker_for_identifier(portal.unique_name + "_move_target")
		move_target_marker.visible_name = destination.formatted_name
		move_target_marker.destination = destination_marker
		move_target_marker.move_to_marker(location_marker)

		if double_ended :
			name = params["type"] + " to " + location.name
			portal = mechanics.Scenery(name=name, location=destination, description=type_description)
			portal_marker = target_state.get_marker_for_identifier(portal.unique_name)
			portal_marker.visible_name = portal.formatted_name
			portal_marker.object = portal
			portal_marker.move_to_marker(destination_marker)
			
			move_target_marker = target_state.get_marker_for_identifier(portal.unique_name + "_move_target")
			move_target_marker.visible_name = location.formatted_name
			move_target_marker.destination = location_marker
			move_target_marker.move_to_marker(destination_marker)
...
#		Leader of the Command staff aboard the Adversity, Captain John Haskell is a stern man. His younger years were spent in the Navy;
#		he had a successful military career in which he eventually captained a destroyer-class star ship during the Martian Secession
#		wars. At the end of end of the war, he retired from the navy with honors. Although he might have lived a peaceful life on Earth,
#		he decided to take a job offer with the non-profit Free Information Society. He was eventually put in command of a star ship once
#		again; the FIS Adversity python-class research vessel. His crew describe him as harsh and distant, and many question the wisdom
#		of putting an ex-military captain in charge of a research vessel. None can deny, however, that he is good at what he does and
#		keeps the crew in line without damaging their morale. He's not open about his motivations for joining the FIS, but he seems
#		driven by his work in a way that few others are. Some suggest, when the captain is out of earshot, that he sees his work for the
#		FIS as some sort of penance for something that happened during the war. The only person who knows for sure is Haskell.
#
